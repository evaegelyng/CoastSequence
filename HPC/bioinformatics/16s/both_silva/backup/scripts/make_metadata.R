...
otu_mat<-read.table("/HPC/bioinformatics/16s/base/results/DADA2_nochim.table", sep="\t", header=T, row.names=1,check.names=F,colClasses="character")

tax_table_silva<-read.table("/HPC/bioinformatics/16s/both_silva/results/assignment.txt", sep='\t', header=T)
...
###Get extraction refs
extraction_refs<-read.table("/lab_metadata/extraction_refs_both_seasons.txt", sep="\t", header=T, row.names=1)
...
###Get PSU refs
PSU_refs<-read.table("/lab_metadata/PSU_refs_both.txt", sep="\t", header=T, row.names=1)
...
write.table(test_md, "/16s/both_silva/results/metadata/metadata_both_dada2.txt", sep="\t", row.names = T, quote=FALSE)

tax_mat_b<-as.matrix(tax_mat_silva[,c("Kingdom","Phylum","Class","Order","Family","Genus")])

write.table(f_otu_mat, "/16s/both_silva/results/otu_silva_dada2.txt", sep="\t", quote=FALSE, row.names=TRUE)
write.table(tax_mat_b, "/16s/both_silva/results/tax_silva_dada2.txt", sep="\t", quote=FALSE, row.names=TRUE)