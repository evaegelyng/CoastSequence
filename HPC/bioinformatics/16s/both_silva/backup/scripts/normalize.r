...
###Load OTU table
otu_mat<-as.matrix(read.table("cleaned_otu_silva_3.txt", sep="\t", header=T, row.names=1,check.names=F))
...
###Load taxonomy table
taxonomy_silva<-read.table("cleaned_tax_silva_3.txt", sep='\t', header=T, comment="")
...
###Load metadata
metadata<-read.table("metadata/cleaned_silva_metadata_3.txt", sep="\t", header=T)
