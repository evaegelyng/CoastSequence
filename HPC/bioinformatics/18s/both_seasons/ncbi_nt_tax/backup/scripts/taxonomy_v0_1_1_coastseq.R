...
MergedTaxIDs<-read.table("/data/MergedTaxIDs", header=TRUE)
...
# Test if the identification is solely based on less than 3 sequences - to detect identifications because of 1 or 2 high-similarity hits, which could be erroneous (07/01/2022)
...
#Add summary column to detect possible.misid based on at least one of the preceding tests (10/01/2022)
...
assign_taxonomy <- function(table,lower_margin=2, remove = c("fungal sp.", "Eukaryota sp.", "Streptophyta sp.", "Opisthokonta sp.", "Dinophyceae sp.", "NA")) {       # removed constant upper margin specification
  pf <- prefilter(table, lower_margin, remove)   # removed constant upper margin specification
  
  # Replace old tax ids with new ones if they have a match in mergedtaxids dataframe
  pf$OldTaxID<-as.numeric(pf$staxid) # Making an extra column on pf, same as staxid but with a new name so it can match with the mergedtaxid dataframe. Added as.numeric to avoid error w. incompatible column classes
  pf_intermediate<-pf %>%
  ...
  cf <- evaluate_classification(gc[[1]])     # gc[2] is a list of the taxids not classified 
  result <- list(classified_table=cf$taxonon_table, all_classifications=cf$all_taxa_table, all_classifications_summed=cf$all_taxa_table_summed, lower=lower_margin, removed=remove)  # removed upper_margin
  if (length(gc[[2]]) != 0) {
    print(paste0("Taxids not found in the classification: ", gc[2])) # Print the list of not matched taxids  
  }
  return(result) 
}
...
prefilter <- function(IDtable, lower_margin=2, remove = c("fungal sp.", "Eukaryota sp.", "Streptophyta sp.", "Opisthokonta sp.", "Dinophyceae sp.", "NA")) {   # removed constant upper_margin specification
  ...
  for (name in ids) {
    ...
    max <- max(test$pident)
    upper <- as.numeric(test$pident.min.best[1]) # set the minimum similarity threshold for including a hit to the minimum similarity of the best matching taxid
...
get_classification <- function(IDtable2) {
  ...
  for (cl in Start_from:o) { # the taxize command "classification" can be run on the all_staxids vector in one line, but often there is
    ...
    #restarted from if it quits)
     tax_match <- classification(all_staxids[cl], db = "ncbi")    
    ...
    # ssciname column with the final identification (19-08-2022)
...
my_classified_result <- assign_taxonomy(IDtable, lower_margin = 2, remove = c("fungal sp.", "Eukaryota sp.", "Streptophyta sp.", "Opisthokonta sp.", "Dinophyceae sp.", "NA")) # removed constant upper_margin specification

tax_table <- my_classified_result$classified_table 

# Add maximum similarity for the best matching taxid to tax_table from summary table (25-01-2022)
...
# Add indication of possible misidentification to tax_table from summary table (25-08-2022)
...
# classification if the dummy taxid is now in use by NCBI (19-08-2022)
